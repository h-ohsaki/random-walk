...
MAX_STEPS = 10000
GRAPH_TYPES = 'random,ba,barandm,ring,tree,btree,lattice,db,3-regular,4-regular,limaini'
...
def simulate(agent_name,
    ...
    for count in range(1, ntrials + 1):
        ...
        while agent.ncovered < n_vertices:
            agent.advance()
            g_index = (g_index + 1) % 100
            agent.graph = graphs[g_index]
            if agent.step > MAX_STEPS:
                naborts += 1
                break
            if attack_interval is not None and agent.step % attack_interval == 0:
                attack_agent(agent)
        stat = status_str(agent, graphs[0], count, naborts, covers, hittings,
                          mean_hittings)
...
def main():
    ...
    for type_ in graph_types.split(','):
        graphs = [create_graph(type_, n_desired, k_desired) for n in range(100)]
        for agent in agent_names.split(','):
            alphas = [0]
            if agent in 'NBRW BiasedRW EigenvecRW ClosenessRW BetweennessRW EccentricityRW':
                alphas = [-.4, -.2, 0, .2, .4]
            for alpha in alphas:
                simulate(agent, graphs, start_vertex, alpha, ntrials,
                         attack_interval)
